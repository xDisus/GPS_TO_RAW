...
class GoogleMaps:
    def __init__(self):
        self.data_locations = ''
        self.dt = pd.DataFrame(data=None)
        self.dt.is_copy = False
    ...
    def to_pandas(self, date_start, date_end, output):
        ...
        for i in tqdm(self.data_locations, desc='Creating database'):
            ...
            if start_date <= date_maps <= end_date:
                self.dt = self.dt.append(i, ignore_index=True)

        self.dt['datetime'] = pd.to_numeric(self.dt['timestampMs'], errors='coerce')
        for i in tqdm(range(self.dt.shape[0]), desc='Converting Date'):
            try:
                self.dt['datetime'][i] = datetime.fromtimestamp(float(self.dt['datetime'][i]) / 1000).strftime(
                    '%d-%m-%Y %H:%M:%S')
            except:
...
if __name__ == '__main__':
    ...
    base.to_pandas('2019-03-17', '2019-03-18', 'out.csv')
